...
class Medoo
{
	...
	// protected const TABLE_PATTERN = "[\p{L}_][\p{L}\p{N}@$#\-_\\.]*";
    protected const TABLE_PATTERN = '([\\p{L}_][\\p{L}\\p{N}@$#\\-_]*|"[^"]+")(\.([\\p{L}_][\\p{L}\\p{N}@$#\\-_]*|"[^"]+"))?';
    ...
    protected const COLUMN_PATTERN = '("([^"]|\\")+"|[\p{L}\p{N}_$#@\-]+)';
    ...
    // not working with "user" table
    /* public function tableQuote(string $table): string
    {
        // Handle legacy manually quoted schema + table, like: "hubcp"."server"
        if (preg_match('/^"[^"]+"\."[^"]+"$/', $table)) {
            return $table; // Already quoted correctly
        }

        // Optional: Normalize `hubcp\".\"server` → hubcp.server
        ...
        // If table has schema prefix (schema.table), quote both parts
    ...
    public function tableQuote(string $table): string
    {
        // Handle fully quoted table or schema.table (e.g., "user" or "schema"."table")
        if (preg_match('/^"[^"]+"$/', $table) || preg_match('/^"[^"]+"\."[^"]+"$/', $table)) {
            return $table;
        }

        // Convert old hubcp\".\"server format to hubcp.server
        $table = preg_replace('/\\\"\\.\\"/', '.', $table);
        $table = str_replace(['\\"', '"'], '', $table);

        // Validate using updated TABLE_PATTERN (must already be updated as discussed)
        if (!preg_match('/^' . $this::TABLE_PATTERN . '$/u', $table)) {
            throw new InvalidArgumentException("Incorrect table name: {$table}.");
        }
        ...
        // Handle reserved words like `user` (quote always)
        return '"' . $this->prefix . $table . '"';
    }
    ...
    public function columnQuote($string): string
    {
        if ($string === '*') {
            return '*';
        }
        ...
        if (str_contains($string, ' AS ')) {
            [$name, $alias] = preg_split('/\s+AS\s+/i', $string);
            return $this->columnQuote($name) . ' AS ' . $this->columnQuote($alias);
        }

        if (str_contains($string, '.')) {
            return implode('.', array_map([$this, 'columnQuote'], explode('.', $string)));
        }

        // Quote all column names regardless of content
     ...
     * @return string|null
     */
    public function insert(string $table, array $values, ?string $primaryKey = null): ?string
    {
        $stack = [];
        $columns = [];
        $fields = [];
        ...
        $isQueryExec = $this->exec($query, $map);
        if($isQueryExec){
            $lastId[] = $this->id();
        }
